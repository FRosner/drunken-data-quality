...
class CheckTest extends FlatSpec with Matchers with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  it should "fail if there are rows that do not satisfy the given condition" in {
    val constraint = Check.satisfies("column > 1")
  ...
  "A satisfies check (Column)" should "succeed if all rows satisfy the given condition" in {
    val constraint = Check.satisfies(new Column("column") > 0)
    val result = ConstraintSuccess("Constraint (column > 0) is satisfied")
    Check(makeIntegerDf(List(1, 2, 3))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "fail if there are rows that do not satisfy the given condition" in {
    val constraint = Check.satisfies(new Column("column") > 1)
    val result = ConstraintFailure("1 rows did not satisfy constraint (column > 1)")
    Check(makeIntegerDf(List(1, 2, 3))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  "A conditional satisfies check" should "succeed if all rows where the statement is true, satisfy the given condition" in {
    val constraint = Check.satisfies((new Column("column1") === 1) -> (new Column("column2") === 0))
    ...
    Check(makeIntegersDf(
      ...
      List(2, 0),
      List(3, 0)
    )).addConstraint(constraint).run().constraintResults shouldBe Map((constraint, result))
  }

  it should "succeed if there are no rows where the statement is true" in {
    val constraint = Check.satisfies((new Column("column1") === 5) -> (new Column("column2") === 100))
  ...
  "A unique key check" should "succeed if a given column defines a key" in {
    val constraint = Check.hasUniqueKey("column1")
    ...
    Check(df).addConstraint(constraint).run().constraintResults shouldBe Map((constraint, result))
  }

  it should "succeed if the given columns define a key" in {
    val constraint = Check.hasUniqueKey("column1", "column2")
  ...
  "An is-always-null check" should "succeed if the column is always null" in {
    val constraint = Check.isAlwaysNull("column")
    val result = ConstraintSuccess("Column column is null")
    Check(makeNullableStringDf(List(null, null, null))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "fail if the column is not always null" in {
    ...
    Check(makeNullableStringDf(List("a", null, null))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }

  "An is-never-null check" should "succeed if the column contains no null values" in {
    val constraint = Check.isNeverNull("column")
    val result = ConstraintSuccess("Column column is not null")
    Check(makeNullableStringDf(List("a", "b", "c"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "fail if the column contains null values" in {
    ...
    Check(makeNullableStringDf(List("a", "b", null))).addConstraint(constraint).
    run().constraintResults shouldBe Map((constraint, result))
  }

  "A to Int conversion check" should "succeed if all elements can be converted to Int" in {
    val constraint = Check.isConvertibleToInt("column")
    val result = ConstraintSuccess("Column column can be converted to Int")
    Check(makeNullableStringDf(List("1", "2", "3"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "succeed if all elements can be converted to Int or are null" in {
    val constraint = Check.isConvertibleToInt("column")
    val result = ConstraintSuccess("Column column can be converted to Int")
    Check(makeNullableStringDf(List("1", "2", null))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "fail if at least one element cannot be converted to Int" in {
    ...
    Check(makeNullableStringDf(List("1", "hallo", "3"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }

  "A to Double conversion check" should "succeed if all elements can be converted to Double" in {
    val constraint = Check.isConvertibleToDouble("column")
    val result = ConstraintSuccess("Column column can be converted to Double")
    Check(makeNullableStringDf(List("1.0", "2.0", "3"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "succeed if all elements can be converted to Double or are null" in {
    val constraint = Check.isConvertibleToDouble("column")
    val result = ConstraintSuccess("Column column can be converted to Double")
    Check(makeNullableStringDf(List("1", "2.0", null))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  "A to Long conversion check" should "succeed if all elements can be converted to Long" in {
    val constraint = Check.isConvertibleToLong("column")
    val result = ConstraintSuccess("Column column can be converted to Long")
    Check(makeNullableStringDf(List("1", "2", "34565465756776"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  "A to Date conversion check" should "succeed if all elements can be converted to Date" in {
    val constraint = Check.isConvertibleToDate("column", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"))
  ...
  "A foreign key check" should "succeed if the given column is a foreign key pointing to the reference table" in {
    ...
    val constraint = Check.hasForeignKey(ref, "column" -> "column")
    val result = ConstraintSuccess("Columns column->column define a foreign key")
    Check(base).addConstraint(constraint).run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "succeed if the given columns are a foreign key pointing to the reference table" in {
    ...
    val constraint = Check.hasForeignKey(ref, "column1" -> "column1", "column2" -> "column2")
    val result = ConstraintSuccess("Columns column1->column1, column2->column2 define a foreign key")

    Check(base).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "succeed if the given columns are a foreign key pointing to the reference table having a different name" in {
    ...
    val constraint = Check.hasForeignKey(ref, "column1" -> "column1", "column3" -> "column2")
  ...
  "A joinable check" should "succeed if a join on the given column yields at least one row" in {
    ...
    val constraint = Check.isJoinableWith(ref, "column" -> "column")
  ...
  it should "succeed if a join on the given columns yields at least one row" in {
    ...
    val constraint = Check.isJoinableWith(ref, "column1" -> "column1", "column2" -> "column2")
  ...
  it should "succeed if a join on the given columns yields at least one row if the columns have a different name" in {
    ...
    val constraint = Check.isJoinableWith(ref, "column1" -> "column1", "column3" -> "column2")
  ...
  "A check if a column is in the given values" should "succeed if all values are inside" in {
    val constraint = Check.isAnyOf("column", Set("a", "b", "c", "d"))
    val result = ConstraintSuccess("Column column contains only values in Set(a, b, c, d)")
    Check(makeNullableStringDf(List("a", "b", "c", "c"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "succeed if all values are inside or null" in {
    val constraint = Check.isAnyOf("column", Set("a", "b", "c", "d"))
    val result = ConstraintSuccess("Column column contains only values in Set(a, b, c, d)")
    Check(makeNullableStringDf(List("a", "b", "c", null))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }

  it should "fail if there are values not inside" in {
    val constraint = Check.isAnyOf("column", Set("a", "b", "d"))
  ...
  "A check if a column satisfies the given regex" should "succeed if all values satisfy the regex" in {
    val constraint = Check.isMatchingRegex("column", "^Hello")
    val result = ConstraintSuccess("Column column matches ^Hello")
    Check(makeNullableStringDf(List("Hello A", "Hello B", "Hello C"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "succeed if all values satisfy the regex or are null" in {
    val constraint = Check.isMatchingRegex("column", "^Hello")
    val result = ConstraintSuccess("Column column matches ^Hello")
    Check(makeNullableStringDf(List("Hello A", "Hello B", null))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }

  it should "fail if there are values not satisfying the regex" in {
    val constraint = Check.isMatchingRegex("column", "^Hello A$")
  ...
  "Multiple checks" should "produce a constraintResults map with all constraints and corresponding results" in {
    val constraint1 = Check.hasNumRowsEqualTo(3)
    val constraint2 = Check.hasNumRowsEqualTo(2)
    val constraint3 = Check.satisfies("column > 0")
    Check(makeIntegerDf(List(1,2,3))).addConstraint(constraint1).addConstraint(constraint2).addConstraint(constraint3).
      run().constraintResults shouldBe Map(
  ...
  "A to boolean conversion check" should "succeed if column values are true and false only" in {
    val constraint = Check.isConvertibleToBoolean("column")
    val result = ConstraintSuccess("Column column can be converted to Boolean")
    Check(makeNullableStringDf(List("true", "false"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "fail if column values are not true and false only" in {
    ...
    Check(makeNullableStringDf(List("true", "false", "error"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }

  it should "succeed if column values are true/TRUE and false/FALSE if case sensitive is false" in {
    val constraint = Check.isConvertibleToBoolean("column", isCaseSensitive = false)
    val result = ConstraintSuccess("Column column can be converted to Boolean")
  ...
  it should "fail if column values are true/TRUE and false/FALSE if case sensitive is true" in {
    val constraint = Check.isConvertibleToBoolean("column", isCaseSensitive = true)
  ...
  it should "succeed if column values are 1 and 0 only" in {
    val constraint = Check.isConvertibleToBoolean("column", "1", "0")
    val result = ConstraintSuccess("Column column can be converted to Boolean")
    Check(makeNullableStringDf(List("1", "0"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "fail if column values are not 1 and 0 only" in {
    val constraint = Check.isConvertibleToBoolean("column", "1", "0")
    val result = ConstraintFailure("Column column contains 1 rows that cannot be converted to Boolean")
    Check(makeNullableStringDf(List("1", "0", "2"))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
  ...
  it should "succeed if column values are convertible or null" in {
    val constraint = Check.isConvertibleToBoolean("column", "1", "0")
    val result = ConstraintSuccess("Column column can be converted to Boolean")
    Check(makeNullableStringDf(List("1", "0", null))).addConstraint(constraint).
      run().constraintResults shouldBe Map((constraint, result))
  }
